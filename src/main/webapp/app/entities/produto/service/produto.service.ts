...
type RestOf<T extends IProduto | NewProduto> = Omit<T, 'validade'> & {
...
export class ProdutoService {
  ...
  protected convertDateFromClient<T extends IProduto | NewProduto | PartialUpdateProduto>(produto: T): RestOf<T> {
    return {
      ...produto,
      validade: produto.validade?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduto: RestProduto): IProduto {
    return {
      ...restProduto,
      validade: restProduto.validade ? dayjs(restProduto.validade) : undefined,
    };
  }
