...
/**
 * REST controller for managing {@link br.ufpa.castanhal.es2.armazem.domain.Produto}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Produto> result = produtoRepository
            ...
            .map(existingProduto -> {
                ...
                if (produto.getAltura() != null) {
                    existingProduto.setAltura(produto.getAltura());
                }
                if (produto.getLargura() != null) {
                    existingProduto.setLargura(produto.getLargura());
                }
                if (produto.getComprimento() != null) {
                    existingProduto.setComprimento(produto.getComprimento());
                }
                if (produto.getCodigoDeBarra() != null) {
                    existingProduto.setCodigoDeBarra(produto.getCodigoDeBarra());
                }
                if (produto.getTipoDeProduto() != null) {
                    existingProduto.setTipoDeProduto(produto.getTipoDeProduto());
                }
